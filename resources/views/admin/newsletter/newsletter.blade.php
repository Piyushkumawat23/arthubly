...
<div class="nl-page">
    <div class="nl-head">
        <div class="nl-title-wrap"><div class="nl-icon"><i class="fas fa-paper-plane"></i></div><div><h1 class="nl-title">Send Newsletter</h1><p class="nl-sub">Compose and send a newsletter to your subscribers.</p></div></div>
        <a href="{{ route('admin.newsletter.index') }}" class="btn btn-outline-secondary nl-btn"><i class="fas fa-arrow-left mr-1"></i> Subscribers</a>
    </div>
    ...
    <div class="nl-compose">
        <div class="nl-card">
            <div class="nl-card-head"><h2 class="nl-card-title"><i class="fas fa-envelope mr-1"></i> Compose Newsletter</h2></div>
            <form action="{{ route('admin.newsletter.send') }}" method="POST">
                @csrf
                <div class="nl-card-body">
                    <div class="mb-4"><label for="subject" class="nl-label">Subject</label><input type="text" name="subject" id="subject" class="form-control nl-input" placeholder="Enter newsletter subject" required></div>
                    <div class="nl-editor"><label for="message" class="nl-label">Message</label><textarea name="message" id="message" class="form-control" rows="10" placeholder="Type your message here..." required></textarea><small class="nl-help">Write the newsletter message that will be delivered to subscribers.</small></div>
                </div>
                <div class="nl-footer"><a href="{{ route('admin.newsletter.index') }}" class="btn btn-light nl-btn mr-2">Cancel</a><button type="submit" class="btn btn-dark nl-btn"><i class="fas fa-paper-plane mr-1"></i> Send Newsletter</button></div>
            </form>
        </div>
    </div>
</div>
