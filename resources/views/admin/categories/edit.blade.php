...
<div class="cat-page">
    <div class="cat-head">
        <div class="cat-title-wrap"><div class="cat-icon"><i class="fas fa-folder-edit"></i></div><div><h1 class="cat-title">Edit Category</h1><p class="cat-sub">Update category details, media, description and status.</p></div></div>
    ...
    <div class="cat-form-layout">
        <div class="cat-form-card">
            <div class="cat-form-head"><h2 class="cat-card-title"><i class="fas fa-pen mr-1"></i> Edit: {{ $category->name }}</h2></div>
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf @method('PUT')
                <div class="cat-form-body">
                    <div class="mb-3"><label for="name" class="cat-label">Category Name</label><input type="text" name="name" id="name" value="{{ $category->name }}" class="form-control cat-input" required></div>
                    <div class="row">
                        <div class="col-md-6 mb-3"><label for="image" class="cat-label">Category Image</label><input class="form-control cat-input cat-upload" type="file" name="image" id="image" accept="image/*">
                            @if($category->image)<div class="cat-preview"><img src="{{ asset('public/uploads/categories/' . $category->image) }}" alt="Category Image"></div>@endif
                        </div>
                        <div class="col-md-6 mb-3"><label for="icon" class="cat-label">Category Icon</label><input class="form-control cat-input cat-upload" type="file" name="icon" id="icon" accept="image/*">
                            @if($category->icon)<div class="cat-preview icon"><img src="{{ asset('public/uploads/categories/icons/' . $category->icon) }}" alt="Category Icon"></div>@endif
                        </div>
                    </div>
                    <div class="mb-3 cat-editor"><label for="description" class="cat-label">Description</label><textarea name="description" id="description" class="form-control" rows="5">{{ $category->description }}</textarea></div>
                    <div class="mb-1"><label for="status" class="cat-label">Status</label><select name="status" id="status" class="form-control cat-input"><option value="1" {{ $category->status == 1 ? 'selected' : '' }}>Active</option><option value="0" {{ $category->status == 0 ? 'selected' : '' }}>Inactive</option></select></div>
                </div>
                <div class="cat-form-foot"><a href="{{ route('admin.categories.index') }}" class="btn btn-light cat-btn">Cancel</a><button type="submit" class="btn btn-dark cat-btn"><i class="fas fa-save mr-1"></i> Update Category</button></div>
            </form>
        </div>
    </div>
</div>
...
<script>
ClassicEditor.create(document.querySelector('#description'),{toolbar:['heading','|','bold','italic','link','bulletedList','numberedList','blockQuote','insertTable','mediaEmbed','undo','redo']}).catch(error=>console.error(error));
</script>
@endsection